...
// Keep only WinAnsi-safe characters (printable ASCII + Latin-1 supplement)
function safe(str) {
  return (str || '').replace(/[^\x0A\x20-\xFF]/g, '')
}
...
async function buildEmailPdf({ from, subject, textBody, htmlBody, extracted }) {
  ...
  const drawText = (text, { size = 10, bold = false, color = rgb(0.1, 0.1, 0.09) } = {}) => {
    page.drawText(safe(text) || ' ', { x: margin, y, size, font: bold ? fontBold : font, color })
    y -= size + 4
  }
  const drawLine = () => {
    y -= 4
  ...
  // Header
  page.drawText('Recu - Email Receipt', { x: margin, y, size: 16, font: fontBold, color: rgb(0.1, 0.1, 0.09) })
  y -= 22
  ...
  gap(4)
  const rawBody = safe(textBody || stripHtml(htmlBody) || '').trim().slice(0, 3000)
