...
function buildEmailPdf({ from, subject, textBody, htmlBody, extracted }) {
  return new Promise((resolve) => {
    ...
    // Header
    doc.fontSize(18).font('Helvetica-Bold').text('Récu — Email Receipt', { align: 'center' })
    doc.moveDown(0.4)
    doc.fontSize(10).font('Helvetica').fillColor('#888').text('Generated by monrecu.app', { align: 'center' })
    doc.moveDown(1)
    ...
    // Email metadata
    doc.fontSize(11).font('Helvetica-Bold').fillColor('#1A1A18').text('Email Details')
    doc.moveDown(0.4)
    doc.fontSize(10).font('Helvetica').fillColor('#444444')
    doc.text(`From:     ${from || '—'}`)
    doc.text(`Subject:  ${subject || '(no subject)'}`)
    doc.moveDown(0.8)
    ...
    if (hasExtracted) {
      doc.moveTo(50, doc.y).lineTo(545, doc.y).strokeColor('#cccccc').lineWidth(0.5).stroke()
      doc.moveDown(0.8)
      doc.fontSize(11).font('Helvetica-Bold').fillColor('#1A1A18').text('Extracted Data')
      doc.moveDown(0.4)
      doc.fontSize(10).font('Helvetica').fillColor('#444444')
      if (extracted.vendor) doc.text(`Vendor:         ${extracted.vendor}`)
      if (extracted.invoice_date) doc.text(`Date:           ${extracted.invoice_date}`)
      if (extracted.invoice_number) doc.text(`Invoice #:      ${extracted.invoice_number}`)
      if (extracted.subtotal != null) doc.text(`Subtotal:       $${extracted.subtotal} CAD`)
      if (extracted.gst != null) doc.text(`GST:            $${extracted.gst} CAD`)
      if (extracted.qst != null) doc.text(`QST:            $${extracted.qst} CAD`)
      if (extracted.hst != null) doc.text(`HST:            $${extracted.hst} CAD`)
      if (extracted.total != null) doc.fontSize(11).font('Helvetica-Bold').text(`Total:          $${extracted.total} CAD`)
      if (extracted.vendor_gst_number) doc.fontSize(10).font('Helvetica').text(`GST #:          ${extracted.vendor_gst_number}`)
      if (extracted.vendor_qst_number) doc.text(`QST #:          ${extracted.vendor_qst_number}`)
      doc.moveDown(0.8)
    }
    ...
    doc.moveTo(50, doc.y).lineTo(545, doc.y).strokeColor('#cccccc').lineWidth(0.5).stroke()
    doc.moveDown(0.8)
    doc.fontSize(11).font('Helvetica-Bold').fillColor('#1A1A18').text('Email Content')
    doc.moveDown(0.4)
    const body = (textBody || stripHtml(htmlBody) || '').trim().slice(0, 3000)
    doc.fontSize(9).font('Helvetica').fillColor('#333333').text(body || '(no content)', { lineGap: 2 })
